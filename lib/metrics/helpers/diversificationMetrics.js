export function calculateCorrelation(portfolioReturns, benchmarkReturns) {
  ...
  const pVariance = pairs.reduce((sum, p) => sum + Math.pow(p.portfolio - pMean, 2), 0) / (pairs.length - 1)
  ...
  if (pStd < 1e-10 || bStd < 1e-10) return null
  return covariance / (pStd * bStd)
}

export function calculateCointegration(priceSeries1, priceSeries2) {
  if (!priceSeries1 || !priceSeries2 || priceSeries1.length !== priceSeries2.length || priceSeries1.length === 0) return null
